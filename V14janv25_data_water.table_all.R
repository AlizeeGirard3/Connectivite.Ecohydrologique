...
for (i in 1:length(ll.pre)) {
  ...
  if (grepl(SNH[1], ll.pre[i])) {
    ...
    
    {
      ...
      # VÉRIFIER SI minuit est aussi écrit 00:00:01 dans HOBO
      
      
      
      # RENDUE ICI -> ##### boucle de concaténation des données (sonde+puits+année ensemble, sinon autre calibration et graphique disctinct) ----
    ...
    ### création de la liste dans la liste [[i]]  ----
    # noted : <- le fichier du level logger correspondant à la position i; [1] : data (dataframe), [2] : metadata (character string)
    ll.clean[[k]] <- list("data" = ll.cal.k, "metadata" = ll.pre.0.metadata) } 
    # else {
