...
raw.to.clean_cal.data <- function(cal.data.path) { # ne calibre pas encore les données
  cal.data.pre.0 <- read.csv(cal.data.path, sep = ";", dec = ",") %>% dplyr::filter(!measure_status == "rejected") %>% 
    select(!contains("x.archive"))
  cal.data.pre.1 <- cal.data.pre.0 %>% mutate(out.R = round(cal.data.pre.0$pt.haut.cm - ((cal.data.pre.0$pt.bas1.cm+cal.data.pre.0$pt.bas2.cm+cal.data.pre.0$pt.bas3.cm)/3), digits = 1)) # out = (pt haut - moyenne pt bas)
  # vérification de valeurs OUT
  if(all(cal.data.pre.1$out.R == round(cal.data.pre.1$out.long.tuyau.sol.cm, digits = 1), na.rm =T))  { # si TOUS TRUE (fonction any()) = changer nom de out.R et supprimer la mesure entrée manuellement // si FALSE = avertissement
    print("ok")# cal.data.pre.1$out.long.tuyau.sol.cm <- cal.data.pre.1$out.R
  ...
  cal.data.pre <- cal.data.pre.1 %>% select(!c(starts_with("pt."), "out.R")) #"site.uid", "well.uid", "trmnt.uid", "lab.probe.id", "probe.uid", "probe.brand", contains("cal."), "comment", 
